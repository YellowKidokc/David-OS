...
Endpoints:
    ...
    POST /approve           — approve a pending rename + learn from acceptance
    POST /reject            — reject a rename + learn from rejection (optional: correction)
    ...
    POST /learner/summary   — naming learner diagnostics
    POST /learner/export    — export naming preferences to PPK
...
class FISAPIHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path == "/classify":
            ...
            from fis.fmeta import append_history, rename_fmeta_with_file
            ...
            if f and f["proposed_name"]:
                ...
                # CLOSE THE LEARN LOOP: feed approval to naming learner
                try:
                    learner = NamingLearner()
                    features = learner.extract_features(
                        domain=f.get("domain", ""),
                        subjects=f.get("subject_codes", []) or [],
                        slug=f.get("slug", ""),
                        extension=Path(f["file_path"]).suffix,
                        confidence=f.get("confidence", 0),
                        original_name=f["original_name"],
                        file_path=f["file_path"],
                    )
                    learner.learn_approve(features, f["proposed_name"])
                except Exception as e:
                    pass  # Don't let learn failures break approval

                # Update .fmeta
                try:
                    append_history(f["file_path"], "APPROVED",
                                 f"renamed_to={f['proposed_name']}")
                    rename_fmeta_with_file(f["file_path"], f["proposed_name"])
                except Exception:
                    pass
            ...
            file_id = body.get("file_id")
            correction = body.get("correction", "")  # David's preferred name
            if not file_id:
                self._json_response({"error": "file_id required"}, 400)
                return
            from fis.db.models import _db, update_file_status
            from fis.fmeta import append_history
            from fis.naming_learner import NamingLearner
            with _db() as conn:
                with conn.cursor() as cur:
                    cur.execute("SELECT * FROM files WHERE file_id = %s", (file_id,))
                    f = cur.fetchone()
            if f:
                update_file_status(file_id, "rejected", None)

                # Feed rejection to naming learner
                try:
                    learner = NamingLearner()
                    features = learner.extract_features(
                        domain=f.get("domain", ""),
                    ...
                    learner.learn_reject(features, f.get("proposed_name", ""),
                ...
                # Update .fmeta
                try:
                    details = f"rejected={f.get('proposed_name', '')}"
                    if correction:
                        details += f" correction={correction}"
                    append_history(f["file_path"], "REJECTED", details)
                except Exception:
                    pass
        ...
        elif path == "/learner/summary":
            from fis.naming_learner import NamingLearner
            learner = NamingLearner()
            self._json_response(learner.get_summary())

        elif path == "/learner/export":
            from fis.naming_learner import NamingLearner
            learner = NamingLearner()
            ppk = learner.export_to_ppk()
            self._json_response({"status": "exported", "ppk": ppk})
    ...
    def _json_response(self, data, status=200):
        try:
            ...
            pass  # client closed socket before we finished — request was processed OK
